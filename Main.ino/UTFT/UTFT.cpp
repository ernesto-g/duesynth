...
static int stateDrawBitmap;
static int smDrX;
static int smDrY;
static int smDrSX;
static int smDrSY;
static int smDrIndexX;
static int smDrIndexY;
static bitmapdatatype smDrData;
#define STATE_DRAW_BITMAP_IDLE		0
#define STATE_DRAW_BITMAP_DRAWING_Y	1
#define STATE_DRAW_BITMAP_DRAWING_X	2
#define STATE_DRAW_BITMAP_DRAWING_Y2	3
int UTFT::drawBitmapStart(int x, int y, int sx, int sy, bitmapdatatype data)
{
	if(stateDrawBitmap==STATE_DRAW_BITMAP_IDLE)
	{
		smDrX = x;
		smDrY = y;
		smDrSX = sx;
		smDrSY = sy;
		smDrData = data;
		stateDrawBitmap = STATE_DRAW_BITMAP_DRAWING_Y;
		cbi(P_CS, B_CS);
		smDrIndexY=0;
		return 0; // OK
	}
...
int UTFT::drawBitmapRun(void)
{				
	switch(stateDrawBitmap)
	{
		case STATE_DRAW_BITMAP_DRAWING_Y:
		{
			setXY(smDrX, smDrY+smDrIndexY, smDrX+smDrSX-1, smDrY+smDrIndexY);
			smDrIndexX=smDrSX-1;
			stateDrawBitmap = STATE_DRAW_BITMAP_DRAWING_X;		
			break;
		}
		case STATE_DRAW_BITMAP_DRAWING_X:
		{
			unsigned int col;
			if(smDrData!=NULL)
			{
				col=pgm_read_word(&smDrData[(smDrIndexY*smDrSX)+smDrIndexX]);
			...
			if(smDrIndexX<0)
			{
				stateDrawBitmap=STATE_DRAW_BITMAP_DRAWING_Y2;
			}
			break;
		}
		case STATE_DRAW_BITMAP_DRAWING_Y2:
		{
			smDrIndexY++;
			if(smDrIndexY>=smDrSY)
			{
				sbi(P_CS, B_CS);
				stateDrawBitmap=STATE_DRAW_BITMAP_IDLE;
				return 1;
			}
			else
			{
